...
function emptyResponseHint(path: string): string {
  return (
    ...
    'Ambil msToken BARU dari DevTools (Application → Cookies) — token lama cepat kedaluwarsa. ' +
    'Isi juga ttwid + s_v_web_id jika ada, tunggu 1 menit, lalu Verifikasi & Muat lagi.'
  )
}
...
/** Pull a fresh msToken (and ttwid if returned) from TikTok before signed API calls. */
async function refreshSessionTokens(cookieHeader: string): Promise<string> {
  ...
  for (const warmUrl of warmUrls) {
    try {
      ...
      if (set.msToken) next = setCookieInHeader(next, 'msToken', set.msToken)
      if (set.ttwid) next = setCookieInHeader(next, 'ttwid', set.ttwid)
      if (set.msToken) break
    } catch {
      // try next warm URL
    }
  }
...
function mapRepostItems(json: RawRepostList): TikTokRepostItem[] {
  ...
  return (json.aweme_list || [])
    .filter(
      (e): e is NonNullable<typeof e> & { aweme_id: string } =>
        Boolean(e.aweme_id),
    )
    .map((e) => {
...
export async function listReposts(
  ...
  if (!getCookieValue(cookieHeader, 'msToken')) {
    throw new TikTokServiceError(
      'msToken wajib. Salin nilai terbaru dari DevTools → Application → Cookies → msToken.',
    )
  }
  ...
  const strategies: Array<{
    ...
    },
    {
      ...
      params: {
        ...
        post_item_list_request_type: '0',
        secUid,
        ...(getCookieValue(cookieHeader, 'msToken')
  ...
  for (let i = 0; i < strategies.length; i++) {
    ...
    // Re-refresh msToken between strategies if previous was empty
    if (i > 0 && lastEmpty) {
      cookieHeader = await refreshSessionTokens(cookieHeader)
      const ms = getCookieValue(cookieHeader, 'msToken')
      if (ms) strategy.params.msToken = ms
      await sleep(800)
    }

    const url = buildSignedUrl('/api/repost/item_list/', strategy.params, '', {
    ...
    if (res.status === 429 || /ratelimit/i.test(text)) {
      throw new TikTokServiceError(rateLimitMessage('/api/repost/item_list/'), 429)
    }
  ...
  throw new TikTokServiceError(emptyResponseHint('/api/repost/item_list/'), 200)
}
...
export async function listAllReposts(
  ...
  // One shared refresh up front; listReposts will refresh again if needed
  let cookieHeader = await refreshSessionTokens(
    normalizeCookieHeader(cookieInput),
  )
